...
use crate::diagnostics::{inc_nack_responder_event, inc_nack_responder_rtx};
...
impl<P> NackResponderInterceptor<P> {
    ...
    fn handle_nack(
        ...
        let Some(stream) = self.streams.get_mut(&nack.media_ssrc) else {
            inc_nack_responder_event("nack_in", "unknown", "stream_miss");
            return;
        };
        inc_nack_responder_event("nack_in", stream.media, "stream_hit");

        // Queue retransmissions
        for seq in seqs_to_retransmit {
            let Some(original_packet) = stream.send_buffer.get(seq) else {
                inc_nack_responder_event("cache", stream.media, "miss");
                continue;
            };
            inc_nack_responder_event("cache", stream.media, "hit");

            let packet = if let (Some(ssrc_rtx), Some(pt_rtx)) =
                (stream.ssrc_rtx, stream.payload_type_rtx)
            {
                inc_nack_responder_rtx(
                    nack.media_ssrc,
                    ssrc_rtx,
                    original_packet.header.payload_type,
                    pt_rtx,
                );
            ...
            let detail = if stream.ssrc_rtx.is_some() && stream.payload_type_rtx.is_some() {
                "queued_rtx"
            } else {
                "queued_original"
            };
            inc_nack_responder_event("retransmit", stream.media, detail);
...
impl<P: Interceptor> NackResponderInterceptor<P> {
    ...
    fn handle_write(&mut self, msg: TaggedPacket) -> Result<(), Self::Error> {
        ...
        {
            inc_nack_responder_event("cache", stream.media, "store");
    ...
    fn bind_local_stream(&mut self, info: &StreamInfo) {
        ...
        if !stream_supports_nack(info) {
            inc_nack_responder_event("bind_local_stream", media, "no_nack");
        } else if let Some(send_buffer) = SendBuffer::new(self.size) {
            inc_nack_responder_event("bind_local_stream", media, "nack");
            let rtx_detail = if info.ssrc_rtx.is_some() && info.payload_type_rtx.is_some() {
                "rtx"
            } else {
                "no_rtx"
            };
            inc_nack_responder_event("bind_local_stream", media, rtx_detail);
            self.streams.insert(
                info.ssrc,
                LocalStream {
                    send_buffer,
            ...
            inc_nack_responder_event("bind_local_stream", media, "invalid_buffer_size");
        }
    ...
    fn unbind_local_stream(&mut self, info: &StreamInfo) {
        if let Some(stream) = self.streams.remove(&info.ssrc) {
            inc_nack_responder_event("unbind_local_stream", stream.media, "stream");
        }
