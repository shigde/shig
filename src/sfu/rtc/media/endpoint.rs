...
/// [`BoxedInterceptor`]: every peer's peer connection then has the same concrete type,
...
impl RtcEndpointBuilder {
    ...
    pub(crate) fn build(self) -> Result<RtcEndpoint> {
        Ok(RtcEndpoint {
            id: self.id,
...
pub(crate) struct RtcEndpoint {
    ...
    /// [`RtcEndpoint::drive_pending_renegotiation`].
    ...
    /// Whether the SFU has answered the endpoint's first offer, completing the initial round of
    /// SDP negotiation. The SFU must never make the *first* offer — only the endpoint can. Until
    /// the endpoint's first offer is answered (which is also when the SFU learns the endpoint's
    /// codec and RTP-header-extension id assignments), no subscribe re-offer is created, so a
    /// forward's m-lines can adopt ids consistent with the endpoint's own m-lines.
    endpoint_negotiated: bool,

    /// Latest `OnConnectionStateChangeEvent` — tracked so the lobby only forwards media once the
    /// subscriber's transport (ICE + DTLS/SRTP) is ready. See [`RtcEndpoint::is_connected`].
...
pub(crate) enum RtcEndpointEvent {
    SFUEvent(SFUEvent),
    PeerConnectionEvent(RTCPeerConnectionEvent),
}

impl Protocol<TaggedBytesMut, RTCMessage, RtcEndpointEvent> for RtcEndpoint {
    type Rout = RTCMessage;
    type Wout = TaggedBytesMut;
    type Eout = RtcEndpointEvent;
    type Error = Error;
    type Time = Instant;
    ...
    fn handle_event(&mut self, evt: RtcEndpointEvent) -> std::result::Result<(), Self::Error> {
        match evt {
            RtcEndpointEvent::SFUEvent(evt) => self.handle_sfu_event(evt),
            RtcEndpointEvent::PeerConnectionEvent(_) => Ok(()),
        }
    }
            ...
            match evt {
                ...
                RTCPeerConnectionEvent::OnSignalingStateChangeEvent(state) => {
                    ...
                    if state == RTCSignalingState::Stable {
                        if let Err(err) = self.drive_pending_renegotiation() {
                            warn!(
                ...
                other => {
                    ...
                    self.events
                        .push_back(RtcEndpointEvent::PeerConnectionEvent(other));
                }
            }
        }
...
impl RtcEndpoint {
    ...
    /// (`RtcLobby::reconcile` binds them). A track without any SSRC (a bare m-line without
    ...
    /// `OnTrack(OnOpen)` delivers the wire SSRC (`RtcLobby::poll_event` binds it).
    ///
    ...
    /// Add a forwarding sender for another endpoint's publish track. Uses a dedicated
    ...
    ///
    ///   * the endpoint's first offer has been answered (`endpoint_negotiated`) — the SFU never
    ///     makes the *first* offer, so the endpoint's own m-lines (and their codec / extension-id
    ...
    ///
    /// Otherwise this is a no-op; the renegotiation is re-driven when the endpoint's first offer is
    /// answered ([`RtcEndpoint::handle_session_description`]), when the connection returns to stable
    /// (see [`RtcEndpoint::poll_event`]), or when the in-flight offer completes
    /// ([`RtcEndpoint::mark_curr_negotiation_complete`]). This mirrors the browser's rule that a
    /// `negotiationneeded` offer is only created from a stable state.
    fn drive_pending_renegotiation(&mut self) -> Result<()> {
        if !self.endpoint_negotiated
    ...
    fn handle_session_description(
        ...
        if sdp_type == RTCSdpType::Offer {
            ...
            // The peer's first offer is now answered: the initial SDP round is complete and the
            // peer's codecs / extension ids are learned. Only now may the SFU start re-offering
    ...
    fn handle_sfu_event(&mut self, evt: SFUEvent) -> Result<()> {
        if let Some(rtc_lobby_id) = evt.rtc_lobby_id() {
            if rtc_lobby_id != self.rtc_lobby_id {
        ...
        if let Some(endpoint_id) = evt.endpoint_id() {
            ...
            return Err(Error::Other("empty endpoint id".to_string()));
        }
        ...
        match evt {
            SFUEvent::Ok { request_id, .. } => {
                warn!(
                    "{}:[{}/{}] receives ok",
                    request_id, self.rtc_lobby_id, self.id,
                );
            }
            SFUEvent::Err {
                request_id, reason, ..
                ...
                warn!(
                    "{}:[{}/{}] receives err due to {}",
                    request_id, self.rtc_lobby_id, self.id, reason
                );
            }
            SFUEvent::Join {
                request_id,
                ...
                warn!(
                    "{}:[{}/{}] has already joined",
                    request_id, rtc_lobby_id, endpoint_id
                );
            }
            ...
            SFUEvent::Leave {
                ...
                warn!(
                    ...
                    request_id, rtc_lobby_id, endpoint_id, reason
                );
            }
        }
        Ok(())
...
mod tests {
    ...
    #[test]
    fn builds_endpoint_with_custom_media_engine() {
        let mut media_engine = MediaEngine::default();
        media_engine
            .register_default_codecs()
            .expect("default codecs should register");
    ...
    #[test]
    fn builds_endpoint_with_custom_setting_engine() {
        let mut media_engine = MediaEngine::default();
        media_engine
            .register_default_codecs()
            .expect("default codecs should register");
    ...
    #[test]
    fn builds_endpoint_with_interceptor_registry() {
